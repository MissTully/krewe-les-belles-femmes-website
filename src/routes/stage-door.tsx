...
export const Route = createFileRoute("/stage-door")({ component: StageDoorPage });
  ...
  return (
    <article className="pb-20 text-cream">
      <section className="relative isolate overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-b from-velvet/40 via-velvet/55 to-velvet" aria-hidden="true" />
        <div className="relative z-10 mx-auto flex min-h-[min(70vh,36rem)] w-min(62rem,calc(100%-2rem)) flex-col items-center justify-end px-4 pb-16 pt-24 text-center">
          ...
          <h1 className="mt-2 font-display text-[clamp(3rem,8vw,5.8rem)] leading-[0.92] text-[#fff7e9]">Stage Door</h1>
      ...
      <section className="wrap py-16">
        ...
        <h2 className="mt-3 font-display text-[clamp(2.1rem,4.5vw,3.3rem)] text-[#fff7e9]">The season, in five acts.</h2>
        <ol className="mt-10 grid list-none gap-0 p-0">
          {events.map((event, i) => (
            ...
            >
              ...
              <div>
                <p className="m-0 font-display text-2xl text-[#fff7e9]">{event.title}</p>
                <p className="m-0 mt-1 text-sm text-cream/65">{event.where}</p>
              </div>
              <p className="m-0 text-sm tracking-wide text-gold-soft">{event.whenLabel}</p>
            </li>
          ))}
        </ol>
      </section>
      ...
      <section className="wrap grid items-start gap-12 pb-8 md:grid-cols-[0.9fr_1.1fr]">
        <div>
          ...
          <ul className="mt-6 grid list-none gap-2 p-0">
            {houseLines.map((line) => (
              <li key={line} className="border-l-2 border-gold pl-4 font-display text-xl italic text-gold-soft">
                {line}
              </li>
            ))}
          </ul>
      ...
      <p className="wrap mt-10 text-center font-display text-xl italic text-gold-soft">{krewe.boardNote}</p>
    </article>
  );
}
